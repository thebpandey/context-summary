...
- `Other infra:` any additional infrastructure relevant to the next session (DNS, external APIs, third-party services, feature flags)
...
## Worked Example (abbreviated, for shape only)

```
### TASK STATE
Building KodeTrain, a visual project intelligence platform. Planning phase complete and pushed. Story 1.1 (scaffold) done and pushed; Story 1.2 (AES-256-GCM crypto helpers, server-only) is next in queue.
...
Working tree: clean
Pushed: yes
Supabase: project not yet created (needed at Story 1.3)
Vercel: Hobby plan, pointed at repo

### DECISIONS LOCKED
**Product:** BYOK model; free tier 3 projects unlimited everything else; Stripe deferred to V2.
**Architecture:** Result<T> at external API boundaries overrides ECC try/catch (do not weaken). Atomic commits via GitHub Git Data API, not Contents API.
**Tooling:** Tailwind 3.4.19 for MVP, not Tailwind 4.

### ACTIVE GOTCHAS
- sync_baseline_sha (push) and latest_context_commit_sha (conflict) are NOT interchangeable; conflating them blocks conflict resolution permanently.
- Windows: use py not python3; rm -rf fails if CWD is inside target, use cmd //c "rmdir /s /q dirname".
- Stack deviates from architecture (Next 16 / Tailwind 3 vs indicated Next 15 / Tailwind 4); accepted, do not "fix".

### OPEN QUESTIONS
- New Supabase project not created (resolve before Story 1.3).
- deferred-work.md, 9 inherited starter issues (resolve before external launch).
...
### NEXT STEP
```
I'm continuing KodeTrain. Story 1.1 is done and pushed. Story 1.2 (AES-256-GCM crypto helpers, server-only) is next. Run bmad-dev-story for Story 1.2 with manual gates (foundational, crypto underpins all secret storage, full attention required).
```
```
